# External and internal contours detection
import cv2
import numpy as np 
...
for i, c in enumerate(contours):
    if hierarchy[0][i][3] == -1:  # Check if the contour is external
        cv2.drawContours(external_contours, contours, i, (255, 255, 255), -1)  # Fill external contours
...
for i, c in enumerate(contours):
    if hierarchy[0][i][3] != -1:  # Check if the contour is internal
        cv2.drawContours(internal_contours, contours, i, (255, 255, 255), -1)  # Fill internal contours
